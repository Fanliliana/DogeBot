...
const daftar1 = `¡Hola! ${pushname} ${ucapanWaktu} \n\nAntes de usar el bot, verifique primero `
       const daftar2 = '```Haga clic en el botón de abajo para verificar o escriba .verify 8473\n𝐃𝐨𝐠𝐞 𝐁𝐨𝐭```'
       const daftar3 = [
          {
            buttonId: `verify`,
            buttonText: {
...
cmhit.push(command)
    ...
    mess = {
      ...
      error: {
        ...
        Iv: "El enlace es un error ",
      },
      only: {
        group: "Solo en el grupo de chat grupal",
      },
    };
...
async function sendFileFromUrl(from, url, caption, msg, men) {
    ...
    if (isGroup && isAntilink && !mek.key.fromMe) {
      if (budy.includes("://chat.whatsapp.com/")) {
        ...
        reply("ANTILINK DETECTADO !!  LO SENTIMOS SERÁ PATADA 🤑");
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
       txtt =`Hi ${pushname} 👋\n\nSI USTED ESTÁ UTILIZANDO MOD WHATSAPP Y LOS BOTONES NO SE MUESTRAN, ESCRIBA #allmenu`
 ...
 𝑪𝒉𝒂𝒕𝒔 𝒈𝒓𝒖𝒑𝒂𝒍𝒆𝒔 : ${groups.length} 
 𝑪𝒉𝒂𝒕𝒔 𝒑𝒓𝒊𝒗𝒂𝒅𝒐𝒔 : ${privat.length}
 𝑪𝒉𝒂𝒕𝒔 𝑻𝒐𝒕𝒂𝒍𝒆𝒔 : ${totalChat.length}
 𝑽𝒆𝒍𝒐𝒄𝒊𝒅𝒂𝒅 : ${latensie.toFixed(4)} _Second_
 𝑨𝒓𝒄𝒉𝒊𝒗𝒐 : ${runtime(process.uptime())}
 𝑷𝒍𝒂𝒕𝒂𝒇𝒐𝒓𝒎𝒂 : ${os.platform()}`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
prep = await xeon.prepareMessageFromContent(from,{buttonsMessage},{quoted: fkontak})
...
シ︎ 𝑪𝒉𝒂𝒕𝒔 𝒈𝒓𝒖𝒑𝒂𝒍𝒆𝒔 : ${groups.length} 
シ︎ 𝑪𝒉𝒂𝒕𝒔 𝒑𝒓𝒊𝒗𝒂𝒅𝒐𝒔 : ${privat.length}
シ︎ 𝑪𝒉𝒂𝒕𝒔 𝑻𝒐𝒕𝒂𝒍𝒆𝒔 : ${totalChat.length}
シ︎ 𝑽𝒆𝒍𝒐𝒄𝒊𝒅𝒂𝒅 : ${latensie.toFixed(4)} _Second_
シ︎ 𝑨𝒓𝒄𝒉𝒊𝒗𝒐 : ${runtime(process.uptime())}
シ︎ 𝑷𝒍𝒂𝒕𝒂𝒇𝒐𝒓𝒎𝒂 : ${os.platform()}
${readmore}
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
       reply('No Sis')
        sendButMessage(from, `Hola ${pushname} 😸`, `¿Qué quieres responder?`, [
          {
            buttonId: `oklah`,
            buttonText: {
              displayText: `WHY:)`,
...
break
case 'h':
reply('Él / ella se ha ido... que él / ella también se vaya al infierno 😏')
break
case 'okeyx':
reply('💐🌺🌻 DÉ LA BIENVENIDA AL NUEVO MIEMBRO CON ALGUNAS FLORES 🌻🌺💐')
break 
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
               ...
               await sleep(1000)
               haibg =`Hi ${pushname}\nEse es mi dueño, ¿Qué quieres saber?`
               buttons = [{buttonId: `mygithub`,buttonText:{displayText: 'GIT BOT'},type:1},{buttonId:`iggw`,buttonText:{displayText:'WHATSAPP'},type:1}]
